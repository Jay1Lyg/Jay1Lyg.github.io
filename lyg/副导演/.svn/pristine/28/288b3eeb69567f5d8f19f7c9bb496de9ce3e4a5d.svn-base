		...
		function (arg, callback){
			...
			getInfo.getPreAuthCode(data.component_access_token, function (err, ret){
				if(err){
					...
					var url = 'https://mp.weixin.qq.com/cgi-bin/componentloginpage?component_appid='+info.AppID+'&pre_auth_code='+pre_auth_code_info.pre_auth_code+'&redirect_uri='+encodeURIComponent(config.homeEntry.url+'/agent/a_SolveAuthCode');
					console.log('url:'+url);
					console.log('************@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@************');
					callback(null, url);
				}
