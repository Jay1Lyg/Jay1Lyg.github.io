...
*
*/
function searchOrCreateRole (role,roleTag_id,featureTag,skillTag,callback){
	var data = {};
	Role.model.findOne()
	.where('name',role)
	...
	.exec( function (err, ret){
		if(err){
			...
			if(ret == null || ret == {}){
				Role.model.create({name:role,roleTag: roleTag_id}, function (err1, ret1){
					if(err1){
						callback(err1, data);
					}else{
				...
				if(ret.name==role&&ret.roleTag == roleTag_id&& ret.featureTag == featureTag&& ret.skillTag == skillTag){
					data.id = ret._id;
					//data.name = ret.name;
					callback(null, data);
				}else{
...
function isCreateCareerInCrews (obj1, obj2, production_crews_id, openid, callback){
	...
	.where('min_height',obj1.min_height)
	.where('max_height',obj1.max_height)
	.where('role_paycheck',obj1.role_paycheck)
	.where('workingdays',obj1.workingdays)
		...
		if(err){
			...
			if(ret){
				CareerInCrew.model.create({
					...
					age: obj1.select_age,
					shape: obj1.shape,
					min_height: obj1.min_height,
					max_height: obj1.max_height,
					role_paycheck : obj1.role_paycheck,
					workingdays : obj1.workingdays
                    //openid : openid
...
exports = module.exports = function (req, res) { 
	...
	async.waterfall([
		function (callback){
			//根据输入的职位的值来创建Role(进行判断去重)
			searchOrCreateRole(params.role,roletag_id, params.featureTag, params.skillTag,function (err, ret){
				if(err){
					throw new Error(err);
				}else{
					role_obj = ret;
